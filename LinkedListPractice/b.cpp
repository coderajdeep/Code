...
/*
	SELECTION SORT (not stable in nature but we can make it stable)

	1st iteration [0, 0] sorted
	2nd iteration [0, 1] sorted
	3rd iteration [0, 2] sorted
*/
...
// selection sort recursive helper
void selectionSortUtil(int arr[], int n) {
	if(n<=1) {
		return;
	}
	int minIndex = 0;
	...
	swapElement(arr[0], arr[minIndex]);
	selectionSortUtil(arr+1, n-1);
}

// selection sort recursive
vector<int> selectionSortRecursive(vector<int>a) {
	int n = a.size();
	if(n==1) {
		return a;
	}
...
int main() {
	...
	while(t--) {
		...
		vector<int>ans2 = selectionSortRecursive(arr);
		for(int i=0; i<n; ++i) {
			cout<<ans2[i]<<" ";
		}
		cout<<"\n";
